...
public class AStar : MonoBehaviour {
    ...
    //Transform StartPosition;//Starting position to pathfind from
    //Transform TargetPosition;//Starting position to pathfind to

    private void Awake()
    {
        GridReference = GetComponent<GridNodes>();//Get a reference to the game manager
    }
    ...
    List<Node> FindPath(Vector3 a_StartPos, Vector3 a_TargetPos)
    {
        ...
        while(OpenList.Count > 0)//Whilst there is something in the open list
        {
            ...
            {
                FinalPathList = GetFinalPath(StartNode, TargetNode);//Calculate the final path
            }
